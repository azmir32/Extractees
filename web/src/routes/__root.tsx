...
function RootComponent() {
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-background text-foreground">
      {/* Hide marketing elements when inside the app */}
      {!isApp && (
        <>
          ...
          <header className="border-b border-border backdrop-blur supports-[backdrop-filter]:bg-background/70 sticky top-0 z-10">
            <div className="mx-auto max-w-6xl px-4 py-2.5 sm:py-3 flex items-center justify-between gap-4 sm:gap-6">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <SignedIn>
                  <Link to="/dashboard" className="text-sm rounded-[var(--radius)] px-3 py-1.5 border border-border hover:bg-muted">Dashboard</Link>
                  <UserButton />
                </SignedIn>
              </div>
            </div>
      ...
      <main className={isApp ? "flex-1" : "flex-1 mx-auto max-w-5xl w-full px-4 py-4 sm:py-6"}>
        <Outlet />
      </main>
      {!isApp && (
        <>
